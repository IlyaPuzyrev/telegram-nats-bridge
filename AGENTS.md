...
| `github.com/PaulSonOfLars/gotgbot/v2` | Типы Telegram Bot API |
...
- condition: "update.EditedMessage != nil"
  subject:
    type: "string"
...
**Update** представлен как `gotgbot.Update` — типизированная структура из библиотеки [gotgbot](https://github.com/PaulSonOfLars/gotgbot). Это обеспечивает:
- Полную типизацию всех полей Telegram API
- Удобный доступ к полям через точечную нотацию (`update.Message.Text`, `update.CallbackQuery.Data`)
- Корректную работу Expr с проверкой типов

Все поля соответствуют [Telegram Bot API](https://core.telegram.org/bots/api#update):
- `update.Message` — входящее сообщение (`*Message`)
- `update.EditedMessage` — отредактированное сообщение (`*Message`)
- `update.CallbackQuery` — callback запрос (`*CallbackQuery`)
- и т.д.
